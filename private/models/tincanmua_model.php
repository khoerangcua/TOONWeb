...
class TinCanMuaModel {
  ...
  public function LoadChiTietTinCanMua( $idtincanmua ) {
    ...
    $link = "";
    taoKetNoi( $link );
    $result = chayTruyVanTraVeDL( $link, "SELECT * FROM (SELECT bang2.*, tbl_theloai.tentheloai FROM (SELECT tbl_tincanmua.*, tbl_taikhoan.anhnguoidung FROM tbl_tincanmua INNER JOIN tbl_taikhoan ON tbl_tincanmua.idtaikhoan = tbl_taikhoan.idtaikhoan) AS bang2 INNER JOIN tbl_theloai ON bang2.idtheloai = tbl_theloai.idtheloai) AS bang1 WHERE bang1.idtincanmua = $idtincanmua" );
    while ( $rows = mysqli_fetch_assoc( $result ) ) {
      array_push( $tincanmua, $rows );
      break;
    }
